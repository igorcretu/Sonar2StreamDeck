...
A plugin for [StreamDock](https://www.hotspottek.com/) (Ajazz/Mirabox) that talks to Sonar's local API directly, with a couple of details that make it actually pleasant to live with day to day: real device names instead of guesses, custom icons per device, and the ability to hide the devices you never use from the rotation.
...
- Auto-detected icon per device type — headset, laptop/integrated speakers, HDMI/display output, or generic speaker — or upload your **own icon** per device if you'd rather see something custom on the key
- The mic capture device cycles in lockstep with the output, so your input follows your output automatically
- A toast notification confirms what you just switched to, since Sonar's own overlay only reacts to its own app
